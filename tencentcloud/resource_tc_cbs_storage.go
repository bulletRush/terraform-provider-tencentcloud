...
func terminateCbsStorage(storageId string, client *client.Client) *resource.RetryError {
	...
	if jsonresp.Code != 0 {
		if jsonresp.CodeDesc == "IAMInnerError" {
			return resource.RetryableError(fmt.Errorf("IAMInnerError, please retry later"))
		}
		return resource.NonRetryableError(fmt.Errorf(
			"terminate storage error, code:%v, message: %v, codeDesc: %v.",
			jsonresp.Code,
			jsonresp.Message,
			jsonresp.CodeDesc,
		))
